...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
    ...
    <div class="bg-white dark:bg-admin-card rounded-2xl p-5 relative overflow-hidden border border-gray-200 dark:border-white/[0.07]">
        ...
        <div class="text-[12px] text-gray-500 dark:text-slate-500 mt-1">Seluruh Riwayat</div>
    </div>
    ...
    <div class="bg-white dark:bg-admin-card rounded-2xl p-5 relative overflow-hidden border border-gray-200 dark:border-white/[0.07]">
        ...
        <div class="text-[12px] text-gray-500 dark:text-slate-500 mt-1">Aktivitas Terkini</div>
    </div>
    ...
    <div class="bg-white dark:bg-admin-card rounded-2xl p-5 relative overflow-hidden border border-gray-200 dark:border-white/[0.07]">
        ...
        <div class="text-[12px] text-gray-500 dark:text-slate-500 mt-1">Kategori Modul</div>
    </div>
    ...
    <div class="bg-white dark:bg-admin-card rounded-2xl p-5 relative overflow-hidden border border-gray-200 dark:border-white/[0.07]">
        ...
        <div class="text-[12px] text-gray-500 dark:text-slate-500 mt-1">Log Terfilter</div>
    </div>
</div>

{{-- Distribusi per Modul --}}
@if(!empty($stats['modules']))
<div class="bg-white dark:bg-admin-card rounded-2xl p-5 mb-5 border border-gray-200 dark:border-white/[0.07]">
    <h3 class="font-bold text-gray-900 dark:text-slate-100 mb-4 text-xs tracking-wider uppercase font-jakarta">Distribusi per Modul</h3>
...
{{-- Filter --}}
<div class="bg-white dark:bg-admin-card rounded-2xl p-4 mb-5 border border-gray-200 dark:border-white/[0.07]">
    <form method="GET" class="flex flex-wrap items-center gap-3">
        <input type="text" name="search" value="{{ $search }}" placeholder="Cari aktivitas atau nama..."
            class="flex-1 min-w-[180px] px-4 py-2 rounded-xl border bg-white dark:bg-slate-900 border-gray-200 dark:border-slate-700 text-xs text-gray-900 dark:text-slate-200 placeholder-gray-400 dark:placeholder-slate-500 focus:ring-2 focus:ring-violet-500 outline-none">

        <select name="module" class="px-3 py-2 rounded-xl border bg-white dark:bg-slate-900 border-gray-200 dark:border-slate-700 text-xs text-gray-900 dark:text-slate-200 focus:ring-2 focus:ring-violet-500 outline-none">
        ...
        <select name="role" class="px-3 py-2 rounded-xl border bg-white dark:bg-slate-900 border-gray-200 dark:border-slate-700 text-xs text-gray-900 dark:text-slate-200 focus:ring-2 focus:ring-violet-500 outline-none">
        ...
        <input type="date" name="date" value="{{ $date }}"
            class="px-3 py-2 rounded-xl border bg-white dark:bg-slate-900 border-gray-200 dark:border-slate-700 text-xs text-gray-900 dark:text-slate-200 focus:ring-2 focus:ring-violet-500 outline-none">
        ...
        <a href="{{ route('admin.activity-log') }}" class="px-4 py-2 bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:hover:bg-slate-700 text-gray-700 dark:text-slate-300 text-xs font-bold rounded-xl transition-all font-jakarta uppercase tracking-wider">Reset</a>
        @endif
    </form>
</div>
...
{{-- Flash --}}
@if(session('success'))
<div class="bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-500/20 rounded-2xl p-4 mb-4 flex items-center gap-3">
...
<div class="bg-white dark:bg-admin-card rounded-2xl shadow-sm overflow-hidden border border-gray-200 dark:border-white/[0.07]">
    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-white/[0.06]">
        <div>
            <h3 class="font-bold text-gray-900 dark:text-slate-100 font-jakarta text-sm">Log Aktivitas</h3>
    ...
    <div class="py-16 text-center">
        ...
        <h3 class="text-sm font-bold text-gray-700 dark:text-slate-300 mb-1 font-jakarta">Tidak ada log</h3>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-white/[0.06]">
                ...
                <tr class="hover:bg-gray-50/50 dark:hover:bg-white/5 transition-colors">
                    ...
                    <td class="px-4 py-3.5 whitespace-nowrap">
                        <div class="text-xs font-semibold text-gray-800 dark:text-slate-200">{{ $log->created_at->format('d M Y') }}</div>
                        <div class="text-[11px] text-gray-400 dark:text-slate-500 font-mono">{{ $log->created_at->format('H:i:s') }}</div>
                        <div class="text-[10px] text-gray-400 dark:text-slate-600 mt-0.5 font-medium">{{ $log->created_at->diffForHumans() }}</div>
                    </td>
                    {{-- Aktor --}}
                    <td class="px-4 py-3.5">
                        <div class="flex items-center gap-2.5">
                            @if($log->user)
                            <img src="{{ $log->user->avatar_url }}" class="w-7 h-7 rounded-lg object-cover flex-shrink-0 border border-gray-200 dark:border-white/[0.1]">
                            @else
                            <div class="w-7 h-7 bg-gray-100 dark:bg-slate-800 rounded-lg flex items-center justify-center flex-shrink-0 border border-gray-200 dark:border-white/[0.06]">
                            ...
                            @endif
                            <div>
                                <div class="text-[13px] font-semibold text-gray-800 dark:text-slate-200">{{ $log->actor ?? 'System' }}</div>
                                <span class="text-[9.5px] font-bold px-1.5 py-0.5 rounded-full capitalize border
                                    {{ $log->role === 'admin' ? 'bg-violet-100 dark:bg-violet-950/30 text-violet-600 dark:text-violet-400 border-violet-200 dark:border-violet-500/10' :
                                       ($log->role === 'karyawan' ? 'bg-blue-100 dark:bg-blue-950/30 text-blue-600 dark:text-blue-400 border-blue-200 dark:border-blue-500/10' :
                                       ($log->role === 'pelanggan' ? 'bg-emerald-100 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 border-emerald-200 dark:border-emerald-500/10' :
                    ...
                    <td class="px-4 py-3.5 max-w-xs">
                        <p class="text-xs text-gray-700 dark:text-slate-300 leading-relaxed font-medium">{{ $log->description }}</p>
                        @if($log->properties)
                        <details class="mt-1">
                            <summary class="text-xs text-violet-600 dark:text-violet-400 cursor-pointer hover:text-violet-500">Detail data</summary>
                            <pre class="text-[10px] text-gray-500 dark:text-slate-400 mt-1 bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-800 p-2.5 rounded-lg overflow-x-auto">{{ json_encode($log->properties, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
                        </details>
                        @endif
                    </td>
                    {{-- IP --}}
                    <td class="px-4 py-3.5 hidden lg:table-cell">
                        <span class="text-xs font-mono text-gray-400 dark:text-slate-500 font-medium">{{ $log->ip_address ?? '—' }}</span>
                    </td>
                </tr>
                @endforeach
            </tbody>
    ...
    <div class="px-6 py-4 border-t border-gray-100 dark:border-white/[0.06] font-jakarta">
        {{ $logs->links() }}
    </div>
    @endif
</div>
