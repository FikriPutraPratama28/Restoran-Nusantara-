...
<div x-data="{showModal:false,showDeleteModal:false,isEdit:false,deleteId:null,editMember:null,
    ...
    <div class="bg-white dark:bg-admin-card rounded-2xl overflow-hidden border border-gray-200 dark:border-white/[0.07]">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-white/[0.06]">
            <div class="flex items-center gap-2">
                ...
                <h3 class="font-bold text-gray-900 dark:text-slate-100 font-jakarta text-sm uppercase tracking-wider">Tim Kami</h3>
            </div>
        ...
        <div class="p-5 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            ...
            <div class="bg-gray-50 dark:bg-slate-950/40 border border-gray-200 dark:border-slate-800/80 rounded-2xl p-5 text-center group relative overflow-hidden transition-all hover:bg-gray-100 dark:hover:bg-slate-950/60">
                @if($member->image_src)
                ...
                @endif
                <p class="font-bold text-gray-800 dark:text-slate-200 text-[14px] font-jakarta">{{ $member->name }}</p>
                <p class="text-gray-500 dark:text-slate-500 text-xs mt-0.5 font-medium">{{ $member->role }}</p>
                <div class="flex gap-1.5 justify-center mt-3 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button @click="openEdit({{ $member->toJson() }})" class="w-7 h-7 bg-gray-100 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-gray-600 dark:text-slate-300 rounded-lg flex items-center justify-center hover:bg-gray-200 dark:hover:bg-slate-700 transition-all" title="Edit">
            ...
            <div class="col-span-full text-center py-12">
                ...
                <p class="text-gray-400 dark:text-slate-400 text-sm font-medium">Belum ada anggota tim</p>
            </div>
            @endforelse
        </div>
    </div>
    ...
    <div x-show="showModal" x-cloak class="fixed inset-0 z-[80] flex items-center justify-center p-4"
        ...
        <div class="relative bg-white dark:bg-admin-sidebar rounded-2xl w-full max-w-md z-10 max-h-[90vh] flex flex-col border border-gray-200 dark:border-white/10"
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-white/[0.06] flex-shrink-0">
                <h3 class="font-bold text-gray-900 dark:text-slate-100 font-jakarta text-sm uppercase tracking-wider" x-text="isEdit ? 'Edit Anggota Tim' : 'Tambah Anggota Tim'"></h3>
    ...
    <div x-show="showDeleteModal" x-cloak class="fixed inset-0 z-[90] flex items-center justify-center p-4"
        ...
        <div class="relative bg-white dark:bg-admin-sidebar rounded-2xl w-full max-w-sm z-10 p-6 text-center border border-gray-200 dark:border-white/10 shadow-2xl">
            ...
            <h3 class="font-bold text-gray-900 dark:text-slate-100 text-lg mb-2 font-jakarta">Hapus Anggota Tim?</h3>
            <p class="text-gray-500 dark:text-slate-400 text-sm mb-6 font-medium">Data anggota tim akan dihapus permanen.</p>
            <div class="flex gap-3">
                <button @click="showDeleteModal=false" class="flex-1 py-2.5 rounded-xl border border-gray-200 dark:border-slate-700 text-gray-600 dark:text-slate-400 text-xs font-bold hover:bg-gray-50 dark:hover:bg-slate-800 transition-all font-jakarta uppercase tracking-wider">Batal</button>
