...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
    ...
    <div class="bg-admin-card rounded-2xl overflow-hidden group shadow-lg" style="border: 1px solid rgba(255,255,255,0.07);">
        <div class="relative h-40 overflow-hidden cursor-pointer border-b" style="border-bottom-color: rgba(255,255,255,0.06);" @click="openLightbox('{{ $facility->image_src }}')">
        ...
        <div class="p-4 bg-slate-900/30">
            <p class="text-xs text-slate-400 mb-4 font-medium leading-relaxed line-clamp-2 h-8">{{ $facility->description }}</p>
            <div class="flex gap-2">
                <button @click="openEdit({{ $facility->toJson() }})" class="flex-1 text-xs py-2 bg-slate-800 text-slate-300 rounded-lg hover:bg-slate-700 transition-all font-bold uppercase tracking-wider font-jakarta flex items-center justify-center gap-1.5 border border-slate-700">
    ...
    <div class="col-span-3 text-center py-16 bg-admin-card rounded-2xl" style="border: 1px solid rgba(255,255,255,0.07);">
        ...
        <p class="text-slate-400 text-sm font-medium">Belum ada fasilitas. Tambahkan fasilitas pertama!</p>
    </div>
    @endforelse
</div>
...
<div x-show="showModal" x-cloak class="fixed inset-0 z-[80] flex items-center justify-center p-4"
    ...
    <div class="relative bg-admin-sidebar rounded-2xl w-full max-w-lg z-10 max-h-[90vh] flex flex-col border border-white/10 shadow-2xl"
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b flex-shrink-0" style="border-bottom-color: rgba(255,255,255,0.06);">
            <h3 class="font-bold text-slate-100 font-jakarta text-sm uppercase tracking-wider" x-text="isEdit ? 'Edit Fasilitas' : 'Tambah Fasilitas'"></h3>
...
<div x-show="showDeleteModal" x-cloak class="fixed inset-0 z-[90] flex items-center justify-center p-4"
    ...
    <div class="relative bg-admin-sidebar rounded-2xl w-full max-w-sm z-10 p-6 text-center border border-white/10 shadow-2xl">
        ...
        <h3 class="font-bold text-slate-100 text-lg mb-2 font-jakarta">Hapus Fasilitas?</h3>
        <p class="text-slate-400 text-sm mb-6 font-medium">Data fasilitas akan dihapus permanen.</p>
        <div class="flex gap-3">
            <button @click="showDeleteModal=false" class="flex-1 py-2.5 border border-slate-700 text-slate-400 text-xs font-bold hover:bg-slate-800 transition-all font-jakarta uppercase tracking-wider">Batal</button>
