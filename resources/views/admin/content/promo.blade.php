...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
    ...
    <div class="col-span-3 text-center py-16 bg-white dark:bg-admin-card rounded-2xl border border-gray-200 dark:border-white/[0.07]">
        ...
        <p class="text-gray-400 dark:text-slate-400 text-sm font-medium">Belum ada promo. Tambahkan promo pertama!</p>
    </div>
    @endforelse
</div>
...
<div x-show="showModal" x-cloak class="fixed inset-0 z-[80] flex items-center justify-center p-4"
    ...
    <div class="relative bg-white dark:bg-admin-sidebar rounded-2xl w-full max-w-lg z-10 max-h-[90vh] flex flex-col border border-gray-200 dark:border-white/10"
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-white/[0.06] flex-shrink-0">
            <h3 class="font-bold text-gray-900 dark:text-slate-100 text-sm font-jakarta uppercase tracking-wider" x-text="isEdit ? 'Edit Promo' : 'Tambah Promo Baru'"></h3>
        ...
        <div class="overflow-y-auto flex-1">
            <form x-show="!isEdit" method="POST" action="{{ route('admin.content.promo.store') }}" class="p-6 space-y-4">
                ...
                <div class="flex gap-3 pt-2">
                    <button type="button" @click="showModal=false" class="flex-1 py-2.5 rounded-xl border border-gray-200 dark:border-slate-700 text-gray-600 dark:text-slate-400 text-xs font-bold hover:bg-gray-50 dark:hover:bg-slate-800 uppercase tracking-wider transition-all font-jakarta">Batal</button>
...
<div x-show="showDeleteModal" x-cloak class="fixed inset-0 z-[90] flex items-center justify-center p-4"
    ...
    <div class="relative bg-white dark:bg-admin-sidebar rounded-2xl w-full max-w-sm z-10 p-6 text-center border border-gray-200 dark:border-white/10 shadow-2xl">
        ...
        <h3 class="font-bold text-gray-900 dark:text-slate-100 text-lg mb-2 font-jakarta">Hapus Promo?</h3>
        <p class="text-gray-500 dark:text-slate-400 text-sm mb-6 font-medium">Promo ini akan dihapus permanen.</p>
