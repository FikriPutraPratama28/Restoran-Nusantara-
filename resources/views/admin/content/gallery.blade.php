...
<div x-data="{showModal:false,showDelete:false,isEdit:false,deleteId:null,editMember:null,
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        ...
        <div class="bg-admin-card rounded-2xl overflow-hidden group" style="border: 1px solid rgba(255,255,255,0.07);">
            ...
            <div class="p-4" style="border-top: 1px solid rgba(255,255,255,0.06);">
                <div class="flex gap-2">
                    <button @click="openEdit({{ $img->toJson() }})" class="flex-1 text-xs py-2 bg-slate-800 text-slate-300 rounded-lg hover:bg-slate-700 transition-all font-bold uppercase tracking-wider font-jakarta flex items-center justify-center gap-1.5 border border-slate-700">
        ...
        @empty
        <div class="col-span-1 sm:col-span-2 lg:col-span-3 text-center py-16 bg-admin-card rounded-2xl" style="border: 1px solid rgba(255,255,255,0.07);">
            ...
            <div class="text-sm font-bold text-slate-300 mb-1 font-jakarta uppercase tracking-wider">Belum ada foto Momen Bersama</div>
            <p class="text-xs text-slate-500 font-medium">Tambah konten menggunakan tombol Tambah Foto di atas.</p>
        </div>
        @endforelse
    </div>
    ...
    <div x-show="showModal" x-cloak class="fixed inset-0 z-[80] flex items-center justify-center p-4"
        ...
        <div class="relative bg-admin-sidebar rounded-2xl w-full max-w-lg p-6 z-10 border border-white/10"
            x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100">
            <h3 class="font-bold mb-4 text-slate-100 font-jakarta text-sm uppercase tracking-wider" x-text="isEdit ? 'Edit Foto' : 'Tambah Foto'"></h3>
    ...
    <div x-show="showDelete" x-cloak class="fixed inset-0 z-[90] flex items-center justify-center p-4"
        ...
        <div class="relative bg-admin-sidebar rounded-2xl p-6 z-10 w-full max-w-sm text-center border border-white/10">
            ...
            <h3 class="font-bold text-slate-100 text-lg mb-2 font-jakarta">Hapus foto?</h3>
            <p class="text-slate-400 text-sm mb-6 font-medium">Tindakan ini tidak dapat dibatalkan.</p>
            <div class="flex gap-3">
                <button @click="showDelete=false" class="flex-1 py-2.5 rounded-xl border border-slate-700 text-slate-400 text-xs font-bold hover:bg-slate-800 transition-all font-jakarta uppercase tracking-wider">Batal</button>
