...
<div class="bg-admin-card rounded-2xl overflow-hidden" style="border: 1px solid rgba(255,255,255,0.07);">
    <div class="px-6 py-4 border-b flex items-center gap-2" style="border-bottom-color: rgba(255,255,255,0.06);">
        ...
        <h3 class="font-bold text-slate-100 font-jakarta text-sm uppercase tracking-wider">Cerita Kami</h3>
    </div>
    <form method="POST" action="{{ route('admin.content.about.update') }}" enctype="multipart/form-data" class="p-6 space-y-4">
        ...
        <div x-data="{previewUrl:'{{ $about->image_src ?? '' }}',
            ...
            <div x-show="!previewUrl" @click="$refs.fi.click()" class="border-2 border-dashed border-slate-700 hover:border-violet-500/50 rounded-xl p-5 text-center cursor-pointer transition-all bg-slate-950/20">
                ...
                <p class="text-xs text-slate-400 font-bold uppercase tracking-wider font-jakarta">Klik untuk pilih gambar</p>
            </div>
        ...
        <div>
            ...
            <input type="url" name="image_url" value="{{ $about->image_url }}" placeholder="https://..." class="w-full px-4 py-2.5 rounded-xl border bg-slate-900 border-slate-700 text-sm text-slate-200 focus:ring-2 focus:ring-violet-500 outline-none">
        </div>
        ...
        <div>
            ...
            <input type="text" name="title" value="{{ $about->title }}" required class="w-full px-4 py-2.5 rounded-xl border bg-slate-900 border-slate-700 text-sm text-slate-200 focus:ring-2 focus:ring-violet-500 outline-none">
        </div>

        <div>
            <label class="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5 font-jakarta">Paragraf 1</label>
            <textarea name="description_1" rows="3" class="w-full px-4 py-2.5 rounded-xl border bg-slate-900 border-slate-700 text-sm text-slate-200 focus:ring-2 focus:ring-violet-500 outline-none resize-none font-medium">{{ $about->description_1 }}</textarea>
        </div>

        <div>
            <label class="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5 font-jakarta">Paragraf 2</label>
            <textarea name="description_2" rows="3" class="w-full px-4 py-2.5 rounded-xl border bg-slate-900 border-slate-700 text-sm text-slate-200 focus:ring-2 focus:ring-violet-500 outline-none resize-none font-medium">{{ $about->description_2 }}</textarea>
        </div>
        ...
        <div>
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <div class="bg-slate-950/40 border border-slate-800/80 rounded-xl p-3">
                    <input type="text" name="stats[{{ $i }}][value]" value="{{ $stat['value'] }}" placeholder="2019" class="w-full px-3 py-1.5 rounded-lg border bg-slate-900 border-slate-700 text-sm font-bold text-slate-200 focus:ring-2 focus:ring-violet-500 outline-none mb-1.5">
                    <input type="text" name="stats[{{ $i }}][label]" value="{{ $stat['label'] }}" placeholder="Tahun Berdiri" class="w-full px-3 py-1.5 rounded-lg border bg-slate-900 border-slate-700 text-slate-500 text-xs focus:ring-2 focus:ring-violet-500 outline-none">
                </div>
                @endforeach
            </div>
        </div>
