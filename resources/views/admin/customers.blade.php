...
<div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
    ...
    <div class="bg-admin-card rounded-2xl p-5 relative overflow-hidden" style="border: 1px solid rgba(255,255,255,0.07);">
        ...
        <div class="text-[11px] text-slate-600 mt-0.5 font-medium">Terdaftar di database</div>
    </div>
    ...
    <div class="bg-admin-card rounded-2xl p-5 relative overflow-hidden" style="border: 1px solid rgba(255,255,255,0.07);">
        ...
        <div class="text-[12px] text-slate-500 mt-1">Registrasi Baru</div>
        <div class="text-[11px] text-slate-600 mt-0.5 font-medium">Bulan berjalan</div>
    </div>
    ...
    <div class="bg-admin-card rounded-2xl p-5 relative overflow-hidden" style="border: 1px solid rgba(255,255,255,0.07);">
        ...
        <div class="text-[12px] text-slate-500 mt-1">Status Aktif</div>
        <div class="text-[11px] text-slate-600 mt-0.5 font-medium">Siap melakukan reservasi</div>
    </div>
</div>

{{-- Table --}}
<div class="bg-admin-card rounded-2xl shadow-sm overflow-hidden" style="border: 1px solid rgba(255,255,255,0.07);">
    <div class="px-6 py-4 border-b flex items-center justify-between" style="border-bottom-color: rgba(255,255,255,0.06);">
        <h3 class="font-bold text-slate-100 font-jakarta text-sm">Daftar Pelanggan</h3>
        <span class="text-xs text-slate-500">{{ $customers->total() }} total</span>
    </div>
        ...
        <table class="w-full">
            ...
            <tbody class="divide-y" style="divide-color: rgba(255,255,255,0.06);">
                ...
                <tr class="hover:bg-white/5 transition-colors">
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-3">
                            <img src="{{ $customer->avatar_url }}" alt="{{ $customer->name }}"
                                class="w-9 h-9 rounded-xl object-cover flex-shrink-0 border" style="border-color: rgba(255,255,255,0.1);">
                            <div>
                                <p class="font-semibold text-slate-200 text-sm">{{ $customer->name }}</p>
                    ...
                    <td class="px-4 py-4">
                        <span class="text-xs font-semibold px-2.5 py-1 rounded-full {{ $customer->is_active ? 'bg-emerald-950/30 text-emerald-400 border border-emerald-500/20' : 'bg-rose-950/30 text-rose-400 border border-rose-500/20' }}">
    ...
    <div class="px-6 py-4 border-t" style="border-top-color: rgba(255,255,255,0.06);">
        {{ $customers->links() }}
    </div>
    @endif
</div>
