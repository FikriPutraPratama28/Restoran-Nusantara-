...
<div x-data="{previewUrl:'',isDragging:false,
    ...
    <div x-show="previewUrl" class="relative mb-3 rounded-xl overflow-hidden h-36 group">
        ...
        <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
            ...
            <button type="button" @click="previewUrl=''" class="bg-red-500 text-white text-xs font-semibold px-3 py-1.5 rounded-xl">🗑️ Hapus</button>
        </div>
    </div>
...
<div class="grid grid-cols-3 gap-4">
    <div>
        ...
        <input type="text" name="icon" :value="editFacility?.icon||'🏢'" placeholder="❄️" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-violet-500 outline-none text-center text-xl">
    </div>
